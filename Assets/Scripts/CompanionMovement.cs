...
public class CompanionMovement : MovementAbstract
{
    ...
    public void UpdateDirection(bool isLeft){
        this.isLeft = isLeft;
        if(isLeft)          { animHandler.SetLeft();  }
        else                { animHandler.SetRight(); }
    }
    ...
    void LateUpdate()
    {
        if(gm.moving)   { animHandler.SetMove();     }
        else            { animHandler.SetIdle();     }
    ...
    void FixedUpdate() {
        if(gm.jumping) { animHandler.SetJump();     }
        else           { animHandler.SetGrounded(); }
